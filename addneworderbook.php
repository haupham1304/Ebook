...
require_once ('dbhelp.php');
	...
	if (!empty($_POST)) {
		...
		$sql = 'select * from publisher where NamePublisher =' .$_POST['pname'];
    	$storeList = executeResult($sql);
		if ($storeList == NULL) {
        	$error['NamePublisher'] = "Tên nhà xuất bản bạn chọn không tồn tại trên hệ thống";
		...
		if (empty($error)) {
			$sql = "insert into storagetypebook(ONamePublisher, OIDStaff, OrderedTime) value('$s_pname', '$s_sid', '$s_time')";
			execute($sql);
...
<html>
<head>
	<title>Add New Order Book</title>
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h2 class="text-center">Thêm Thông tin đặt sách mới</h2>
			</div>
