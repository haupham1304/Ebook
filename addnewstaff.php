...
require_once ('dbhelp.php');
	...
	if (!empty($_POST)) {
    	...
    	$sql = 'select * from staff where IDStaff =' .$_POST['id'];
    	$staffList = executeResult($sql);
    	if ($staffList != NULL) {
        	$error['IDstaff'] = "Mã nhân viên này đã tồn tại trên hệ thống";
		...
		if (empty($error)) {
			$sql = "insert into staff(Fname,Mname,Lname,AddressStaff,EmailStaff,PhoneStaff,PasswordStaff,UserStaff,IDStaff,SIDStorage) value('$s_fname', '$s_mname', '$s_lname', '$s_address', '$s_email', '$s_phone', '$s_user', '$s_pass', '$s_id', '$s_sid')";
			execute($sql);
