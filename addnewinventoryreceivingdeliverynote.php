...
require_once ('dbhelp.php');
	...
	if (!empty($_POST)) {
        ...
        $sql = 'select * from inventoryreceivingdeliverynote where IDNote =' .$_POST['nid'];
    	$idList = executeResult($sql);
    	if ($idList != NULL) {
        	$error['IDNote'] = "Mã nhân phiếu này đã tồn tại trên hệ thống";
		...
		if (empty($error)) {
			$sql = "insert into inventoryreceivingdeliverynote(IDNote,NameStaff,Time,StoreAddress,NameOfBook,IDOfBook,AmountOfBook,CurrentPriceOfBook,IDIDStaff) value('$s_id', '$s_sname', '$s_time', '$s_address', '$s_bname', '$s_bid', '$s_ammount', '$s_price', '$s_sid')";
			execute($sql);
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h2 class="text-center">Thêm phiếu nhập xuất mới</h2>
			</div>
