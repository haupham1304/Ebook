...
require_once ('dbhelp.php');
	...
	if (!empty($_POST)) {
		...
		$sql = 'select * from author where IDAuthor =' .$_POST['aid'];
    	$storeList = executeResult($sql);
		if ($storeList == NULL) {
        	$error['IDAuthor'] = "Mã tác giả bạn chọn không tồn tại trên hệ thống";
		...
		if (empty($error)) {
			$sql = "insert into contact(CIDAuthor, CIDStaff) value('$s_auid', '$s_staid')";
			execute($sql);
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h2 class="text-center">Thêm liên hệ mới</h2>
			</div>
