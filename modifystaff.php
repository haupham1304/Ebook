...
require_once ('dbhelp.php');
	...
	if (!empty($_POST)) {
		...
		if (empty($error)) {
			$sql = "update staff set Fname = '$s_fname', Mname = '$s_mname', Lname = '$s_lname', AddressStaff = '$s_address', EmailStaff = '$s_email', PhoneStaff = '$s_phone', UserStaff = '$s_user', PasswordStaff = '$s_pass', SIDStorage = '$s_sid' where IDStaff = " .$s_id;
			execute($sql);
	...
	if (isset($_GET['id'])) {
		...
		if ($staffList != null) {
			...
			$ss_mname    = $std['Mname'];
			$ss_lname    = $std['Lname'];
			$ss_address  = $std['AddressStaff'];
			$ss_email    = $std['EmailStaff'];
			$ss_phone    = $std['PhoneStaff'];
			$ss_user     = $std['UserStaff'];
			$ss_pass     = $std['PasswordStaff'];
			$ss_sid      = $std['SIDStorage'];
		} else {
			$id = '';
		}
	}
